...
mod tests {
    ...
    fn test_log_format_text() {
        ...
        let drain = FastJobFormat::new(decorator).fuse();
        let logger = slog::Logger::root_typed(drain, slog_o!()).into_erased();

        log_format_cases(logger);

        let expect = r#"[2019/01/15 13:40:39.619 +08:00] [INFO] [lib.rs:469] []
[2019/01/15 13:40:39.619 +08:00] [INFO] [lib.rs:469] [Welcome]
[2019/01/15 13:40:39.619 +08:00] [INFO] [lib.rs:470] ["Welcome TiKV"]
[2019/01/15 13:40:39.619 +08:00] [INFO] [lib.rs:471] [欢迎]
[2019/01/15 13:40:39.619 +08:00] [INFO] [lib.rs:472] ["欢迎 TiKV"]
[2019/01/15 13:40:39.615 +08:00] [INFO] [lib.rs:455] ["failed to fetch URL"] [backoff=3s] [attempt=3] [url=http://example.com]
[2019/01/15 13:40:39.619 +08:00] [INFO] [lib.rs:460] ["failed to \"fetch\" [URL]: http://example.com"]
[2019/01/15 13:40:39.619 +08:00] [DEBUG] [lib.rs:463] ["Slow query"] ["process keys"=1500] [duration=123ns] [sql="SELECT * FROM TABLE WHERE ID=\"abc\""]
[2019/01/15 13:40:39.619 +08:00] [WARN] [lib.rs:473] [Type] [Other=-inf] [Score=inf] [Counter=NaN]
[2019/01/16 16:56:04.854 +08:00] [INFO] [lib.rs:391] ["more type tests"] [str_array="[\"💖\", \"�\", \"☺☻☹\", \"日a本b語ç日ð本Ê語þ日¥本¼語i日©\", \"日a本b語ç日ð本Ê語þ日¥本¼語i日©日a本b語ç日ð本Ê語þ日¥本¼語i日©日a本b語ç日ð本Ê語þ日¥本¼語i日©\", \"\\\\x80\\\\x80\\\\x80\\\\x80\", \"<car><mirror>XML</mirror></car>\"]"] [u8=34] [is_None=None] [is_false=false] [is_true=true] ["store ids"="[1, 2, 3]"] [url-peers="[\"peer1\", \"peer 2\"]"] [urls="[\"http://xxx.com:2347\", \"http://xxx.com:2432\"]"] [field2="in quote"] [field1=no_quote]
"#;
...
"#;
    ...
    fn test_slow_log_dispatcher() {
        let normal = FastJobFormat::new(PlainSyncDecorator::new(NormalWriter));
        let slow = FastJobFormat::new(PlainSyncDecorator::new(SlowLogWriter));
        // let rocksdb = FastJobFormat::new(PlainSyncDecorator::new(RocksdbLogWriter));
        // let raftdb = FastJobFormat::new(PlainSyncDecorator::new(RaftDBWriter));
        // let drain = LogDispatcher::new(normal, rocksdb, raftdb, Some(slow)).fuse();
        let drain = LogDispatcher::new(normal, Some(slow)).fuse();
        let drain = SlowLogFilter {
            threshold: 200,
            inner: drain,
        }
